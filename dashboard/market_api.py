...
# Short in-process cache for the Markets Overview endpoint.
# This reduces repeated Yahoo Finance calls when several users
# refresh the overview at nearly the same time.
MARKET_SNAPSHOT_CACHE = {}
MARKET_SNAPSHOT_CACHE_TTL = 20
...
def get_market_snapshot(symbol):
    """
    Return the latest available market price and status.

    This helper contains the Yahoo Finance logic used by both
    the single-symbol live-price API and Markets Overview.
    It avoids making one Django view call another internally.
    """
    ...
    if cached is not None:
        ...
        if time.time() - cached_at < MARKET_SNAPSHOT_CACHE_TTL:
    ...
    intraday = clean_dataframe(intraday)
    ...
    if not intraday.empty:
        ...
        latest_intraday_timestamp = intraday.index[-1]

    intraday_is_recent = False

    if latest_intraday_timestamp is not None:
        now = pd.Timestamp.now(tz="UTC")
        candle_time = pd.Timestamp(latest_intraday_timestamp)
        ...
        intraday_is_recent = age_minutes <= 30

    if (
        intraday_is_recent
        and latest_intraday_price is not None
    ):
        price = latest_intraday_price
    ...
    elif latest_intraday_price is not None:
        price = latest_intraday_price
        price_source = "latest_available"
        market_open = False
        market_state = "CLOSED"
...
def markets_overview(request):
    ...
    market_names = {
        "XAUUSD": {"name": "Gold / U.S. Dollar", "type": "Spot Gold"},
        "EURUSD": {"name": "Euro / U.S. Dollar", "type": "Forex"},
        "GBPUSD": {"name": "British Pound / U.S. Dollar", "type": "Forex"},
        "BTCUSD": {"name": "Bitcoin / U.S. Dollar", "type": "Crypto"},
    }
    ...
    for symbol in SYMBOL_MAP:
        try:
            ...
            markets.append({
                "symbol": symbol,
                "name": market_names[symbol]["name"],
                "type": market_names[symbol]["type"],
                **snapshot,
            })
